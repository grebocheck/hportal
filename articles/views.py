...
from articles.models import Article, Review, Bookmark
...
class ArticleCreateView(LoginRequiredMixin, CreateView):
    model = Article
    fields = ['title', 'cover_image', 'body', 'file_1', 'file_2', 'file_3', 'featured']
...
def submitReview(request):
    if request.method == "POST":
        ...
        return redirect(
            request.META['HTTP_REFERER'] + '#reviews')  # request.META['HTTP_REFERER'] -> redirects to same page


class ReviewUpdateView(LoginRequiredMixin, UpdateView):
    model = Review
    fields = ['title', 'body']
    template_name = 'articles/review-update.html'

    def get_queryset(self):
        queryset = super(ReviewUpdateView, self).get_queryset()
        queryset = queryset.filter(user=self.request.user)
        return queryset
...
def reviewUpdate(request, pk):
    if request.method == "POST":
        ...
        return render(request, 'articles/review-update.html', context)


class ReviewDeleteView(LoginRequiredMixin, DeleteView):
    model = Review
    template_name = 'articles/review-delete.html'
